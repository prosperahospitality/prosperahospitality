...
const plans = [
  {
    ...
    plan: (
      <>
        ...
        <div className='mt-8'>
          <Button color="default" fullWidth radius='none' className='bg-[#800000] text-white font-semibold py-3'>
            Choose Your Plan
          </Button>
        </div>
      </>
    ...
    <section className="w-[90%] lg:w-[80%] m-auto py-10">
      ...
      <div className="my-10 flex lg:grid lg:grid-cols-3 w-full overflow-x-auto snap-x snap-mandatory scroll-smooth gap-5 lg:gap-6">
        {plans.map((plan, index) => (
          <div 
            key={index} 
            className={`flex-shrink-0  border rounded-lg shadow-lg overflow-hidden snap-start ${plan.name === 'Standard' ? 'border-[#800000] relative' : 'border-gray-100'}`}
          >
